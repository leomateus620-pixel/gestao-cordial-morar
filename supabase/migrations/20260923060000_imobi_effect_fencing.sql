-- Um resultado HTTP pode chegar depois de o lease ou a decisão de publicação
-- mudar. O checkpoint anterior ao POST e as confirmações finais são cercados
-- pela posse ATUAL do job, pela conta e pela intenção de disponibilidade.
...
-- Ocultar/retirar o imóvel inteiro não pode separar a decisão local da outbox.
-- O conjunto de contas é lido sob a trava do imóvel. Uma criação ambígua sem
-- código remoto também entra, pois a remoção terá de reconciliar por referência.
...
-- A galeria herdou políticas WITH CHECK (true). Limita escritas aos mesmos
-- perfis que já podem alterar o imóvel; RPCs transacionais de galeria seguem
-- disponíveis via service_role e validam a revisão esperada.
...
-- O cron de importação existente só drenava jobs já iniciados pela interface.
-- Semeia um ciclo incremental por conta, de forma transacional e espaçada.
...
BEGIN
  FOR _job IN SELECT jobid FROM cron.job
    WHERE jobname IN ('property-import-seed-cordial', 'property-import-seed-morar')
  LOOP
...
SELECT cron.schedule(
  'property-import-seed-cordial', '11 3 * * *',
  $job$SELECT public.property_import_seed_incremental('cordial'::public.imobi_provider)$job$
);
SELECT cron.schedule(
  'property-import-seed-morar', '41 3 * * *',
  $job$SELECT public.property_import_seed_incremental('morar'::public.imobi_provider)$job$
);
