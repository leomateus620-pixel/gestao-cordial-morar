...
-- Atomic at-most-once claim for notification-originated email enqueueing. The
-- queue/provider idempotency key remains a second, independent guard.
...
-- The private Broadcast below is the only realtime transport for notifications.
-- Remove the legacy full-row Postgres Changes publication configured previously.
DO $$
BEGIN
...
-- Private Broadcast authorizes only the channel belonging to the JWT subject.
-- The websocket payload contains an opaque notification id; display data is
-- fetched afterwards from the scoped inbox RPC.
DO $$
BEGIN
...
-- Tenant access is fail-closed. Users without reliable historical evidence stay
-- unscoped until an administrator completes onboarding explicitly.
...
-- Entity policies use the same tenant boundary as the inbox so a copied deep
-- link cannot bypass notification visibility.
...
-- Inbox reads and read-state mutations are intentionally RPC-only. This keeps
-- pagination and the payload allowlist at one auditable boundary.
...
-- The first-open mutation keeps the operational event but never returns or logs timing to brokers.
...
-- Correct aggregate computation: every assignment contributes exactly once.
