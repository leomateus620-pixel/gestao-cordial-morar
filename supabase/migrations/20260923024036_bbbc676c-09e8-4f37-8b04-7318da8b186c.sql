-- 23/09/2026 — Opção 1 aprovada: versão incremental da 20260923032000 SEM o
-- UPDATE em massa. Fotos antigas ficam como legado explícito (tabela à parte),
-- sem reprocessar, sem reenviar e sem tocar em property_images (evita subir a
-- revisão da galeria e disparar reenvio).

-- 1) Snapshot (só inserção) do estado antes da correção.
CREATE TABLE IF NOT EXISTS public.backup_20260923_property_images AS
  SELECT now() AS snapshot_at, i.* FROM public.property_images i;
CREATE TABLE IF NOT EXISTS public.backup_20260923_image_links AS
  SELECT now() AS snapshot_at, l.* FROM public.property_image_provider_publications l;
CREATE TABLE IF NOT EXISTS public.backup_20260923_sync_jobs AS
  SELECT now() AS snapshot_at, j.* FROM public.property_sync_jobs j
   WHERE j.status IN ('pending','processing','retry','failed');
CREATE TABLE IF NOT EXISTS public.backup_20260923_publications AS
  SELECT now() AS snapshot_at, p.* FROM public.property_provider_publications p;
GRANT ALL ON public.backup_20260923_property_images, public.backup_20260923_image_links,
  public.backup_20260923_sync_jobs, public.backup_20260923_publications TO service_role;
ALTER TABLE public.backup_20260923_property_images ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.backup_20260923_image_links ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.backup_20260923_sync_jobs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.backup_20260923_publications ENABLE ROW LEVEL SECURITY;

-- 2) Coluna de intenção (nula = legado, sem intenção gravada).
...
-- 3) Legado explícito e lista de análise (não é convergência).
CREATE TABLE IF NOT EXISTS public.property_image_legacy_review (
  image_id uuid PRIMARY KEY REFERENCES public.property_images(id) ON DELETE CASCADE,
  property_id uuid NOT NULL,
  category text NOT NULL CHECK (category IN ('legacy_unverified','variant_mismatch','watermark_v1')),
  current_hash text,
  expected_hash text,
  review_status text NOT NULL DEFAULT 'open' CHECK (review_status IN ('open','queued','recovered','dismissed')),
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);
GRANT SELECT ON public.property_image_legacy_review TO authenticated;
GRANT ALL ON public.property_image_legacy_review TO service_role;
ALTER TABLE public.property_image_legacy_review ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Admin lê revisão de fotos legadas" ON public.property_image_legacy_review
  FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'admin'::public.app_role));

INSERT INTO public.property_image_legacy_review (image_id, property_id, category, current_hash, expected_hash)
SELECT i.id, i.property_id,
       CASE WHEN i.destination_hash IS NULL THEN 'legacy_unverified'
            WHEN i.destination_hash LIKE '%@v1' THEN 'watermark_v1'
            ELSE 'variant_mismatch' END,
       i.destination_hash, public.property_expected_watermark_hash(p.publish_targets)
  FROM public.property_images i JOIN public.properties p ON p.id = i.property_id
 WHERE NOT COALESCE(i.pending_remote_delete, false)
   AND i.destination_hash IS DISTINCT FROM public.property_expected_watermark_hash(p.publish_targets)
ON CONFLICT (image_id) DO NOTHING;

-- 4) Mesmas funções/gatilhos da 032000 (sem o UPDATE em massa).
...
BEGIN
  ...
  UPDATE public.property_image_legacy_review SET review_status = 'queued', updated_at = now()
   WHERE property_id = NEW.id AND review_status = 'open';
  RETURN NEW;
END $$;
...
-- Scanner: legado (intenção nula) nunca é candidato.
...
BEGIN
  ...
  UPDATE public.property_image_jobs SET status = 'succeeded', lease_expires_at = NULL,
   ...
   WHERE id = _job_id;
  UPDATE public.property_image_legacy_review SET review_status = 'recovered', updated_at = now()
   WHERE image_id = _job.image_id;
  RETURN true;
END $$;
